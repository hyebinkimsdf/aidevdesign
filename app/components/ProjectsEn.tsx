...
import { siteProjectsEn, type SiteProject, type SiteProjectThumb } from "@/lib/site-projects";
        ...
        <div className="grid grid-cols-[1.2fr_0.9fr] gap-4">
          <div className="rounded-[1.75rem] border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-5 rounded-2xl bg-white/6 p-3">
              {["Name", "Phone", "Purchase Type"].map((label) => (
    ...
    >
      ...
      <div className="relative px-5 py-5">
        <div className="mb-4">
          ...
          <h4 className="mt-2 text-xl font-semibold text-white">Global MIG</h4>
        </div>
        <div className="grid grid-cols-3 gap-2 mb-3">
          {["E-commerce", "Corporate", "Global"].map((label) => (
    ...
    >
      ...
      <div className="relative px-5 py-4">
        <div className="mb-3">
          ...
          <h4 className="mt-1.5 text-xl font-semibold text-white">Vivid Drone</h4>
        </div>
        <div className="grid grid-cols-3 gap-2 mb-3">
          {["Drone", "Music Video", "CF · Broadcast"].map((label) => (
            <div key={label} className="rounded-xl border border-white/10 bg-white/5 p-2.5 text-center">
...
export default function ProjectsEn() {
  ...
  const filtered = siteProjectsEn.filter((p) => {
    const matchCategory =
    ...
    >
      <div className="mx-auto max-w-4xl">
        ...
        <h2 className="mb-4 max-w-2xl text-2xl font-semibold leading-snug text-foreground sm:text-3xl">
          Focused on live production sites —
          <br />
          organised to show scope and service purpose.
        </h2>
        <p className="mb-10 max-w-2xl text-sm leading-6 text-muted/75">
          Projects where I owned not just screen design but also conversion flow, information architecture, and ongoing updates.
          <br />
          <strong className="font-bold text-foreground">All live services</strong> — organised by domain, with live links and repos where available.
        </p>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="flex h-40 items-center justify-center rounded-2xl border border-border">
            <p className="font-mono text-xs text-muted/70">No results found.</p>
          </div>
        )}
      </div>
    </section>
