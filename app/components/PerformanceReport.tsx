...
const categoryColor: Record<string, string> = {
  렌더링: "text-sky-400 bg-sky-400/10 border-sky-400/20",
  ...
  접근성: "text-accent bg-accent/10 border-accent/20",
};
    ...
    >
      ...
      <div className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border border-border bg-surface p-6 shadow-2xl">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {item.tags.map((t) => (
            <span key={t} className={`inline-block rounded border px-2 py-0.5 font-mono text-xs ${techColor[t] ?? "text-muted bg-surface border-border"}`}>
              {t}
            </span>
          ))}
        </div>
        ...
        <p className="mt-2 text-sm text-muted">{item.cause}</p>
        ...
        {(item.beforeCode || item.afterCode) && (
          <div className="mt-5 space-y-3">
            {item.beforeCode && (
              <div>
                <p className="mb-1 font-mono text-xs text-muted">변경 전</p>
            ...
            {item.afterCode && (
              <div>
                <p className="mb-1 font-mono text-xs text-accent">변경 후</p>
    ...
    >
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mb-10 text-sm text-muted">
          Lighthouse 측정 기반으로 LCP · INP · 접근성 문제를 진단하고 개선했습니다. 각 항목을 클릭하면 상세 내용을 확인할 수 있습니다.
        </p>
        ...
        <div className="mb-8 rounded border border-border bg-surface/20 p-4">
          <div className="flex items-center justify-between mb-4">
            <p className="font-mono text-xs uppercase tracking-widest text-muted">필터</p>
          ...
          {/* Domain */}
          <div className="mb-4">
            <p className="mb-2 font-mono text-xs text-muted/60">도메인</p>
          ...
          {/* Tech tags */}
          <div>
            <p className="mb-2 font-mono text-xs text-muted/60">기술 스택</p>
            <div className="flex flex-wrap gap-2">
              <button
